...
const API_USER = `https://itch.io/api/1/${process.env.API_KEY_ITCH}/search/users?query=`;
const MAX_GAMES = 10;
...
export default function user(msg, args) {
  ...
  axios.get(`${API_USER}${username}`).then((res) => {
    let user = res.data.users[0];

    axios
      .get(`${API_GAMES}${username}`)
      .then((res) => {
        ...
        games.forEach((g) => {
          ...
          if (isTruncated)
            embed.setDescription(`Showing only first ${MAX_GAMES} results.`);
